...
PARAM  <- list()
PARAM$experimento <- "FE9250_EXP_CC_08"
...
TendenciaYmuchomas  <- function( dataset, cols, ventana=3, tendencia=TRUE, minimo=FALSE, maximo=FALSE, promedio=TRUE, 
...
CanaritosAsesinos  <- function( canaritos_ratio=0.8 )
{
  gc()
  dataset[ , clase01:= ifelse( clase_ternaria=="CONTINUA", 0, 1 ) ]
...
{
  ncol( dataset )
  CanaritosAsesinos( canaritos_ratio = 0.8 )
  ncol( dataset )
}
